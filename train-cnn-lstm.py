...
x = Conv1D(nb_filters, 5, activation="relu", padding='valid')(x)
x = MaxPooling1D(2)(x)
x = Conv1D(nb_filters, 5, activation="relu", padding='valid')(x)
x = MaxPooling1D(2)(x)
...
checkpoint = ModelCheckpoint(filepath='weights.ep{epoch:02d}-acc{val_acc:.3f}.hdf5', monitor='val_acc', verbose=1, save_best_only=True)
